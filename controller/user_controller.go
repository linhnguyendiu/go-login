...
import (
	"connect-mysql/db"
	format_errors "connect-mysql/format-errors"
	"connect-mysql/models"
	"connect-mysql/pagination"
	validations "connect-mysql/validations"
	"net/http"
	"os"
	"strconv"
	"time"
...
func GetTrashedUsers(c *gin.Context) {
	...
	result, err := pagination.Paginate(db.DB.Unscoped().Where("deleted_at IS NOT NULL"), page, perPage, nil, &users)
	if err != nil {
		format_errors.InternalServerError(c)
		return
	}
